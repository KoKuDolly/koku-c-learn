...
您好，这是Bing。我可以帮您回答关于翻译单元内部和外部的问题。😊

翻译单元是一个包含源代码，头文件和其他依赖项的文件。¹²³ 每个翻译单元都是由编译器单独编译的。² 编译完成之后，链接器会将编译的翻译单元合并到一个程序中。²

翻译单元内部和外部是指标识符（变量，函数，常量等）的链接属性，也就是说它们在不同的翻译单元中如何引用或不引用同一实体。¹²³

有两种类型的链接：

- 内部链接：具有内部链接的标识符只能在声明它的翻译单元内部访问。¹²³ 该单元内的任何标识符都可以访问具有内部链接的标识符。¹ 它由关键字static实现。¹²³ 内部链接的标识符存储在RAM的初始化或未初始化段中。¹
- 外部链接：具有外部链接的标识符可以在任何翻译单元中访问。¹²³ 外部链接的标识符在翻译单元之间共享，并且被认为位于程序的最外层。¹ 它是全局作用域变量和函数的默认链接。¹²³ 关键字extern实现外部链接。¹²³

如果您想了解更多关于翻译单元内部和外部的知识，请参考以下链接：

- [C语言必学之内部链接和外部链接 - 知乎](^[1]^)
- [内部链接 (internal linkage)和外部链接 (external linkage)](^[2]^)
- [具有外部链接并且在没有任何先前声明的情况下被定义, const 内部链接, 链接说明符 C++, C中的链接有哪些类型, C 外部引用, C中 ...](^[3]^)

希望这些信息对您有所帮助。如果您还有其他问题，欢迎继续与我交流。😊

源: 与必应的对话， 2023/8/11
(1) C语言必学之内部链接和外部链接 - 知乎 - 知乎专栏. https://zhuanlan.zhihu.com/p/260777861.
(2) 内部链接 (internal linkage)和外部链接 (external linkage). https://blog.csdn.net/tonny0832/article/details/12558493.
(3) 具有外部链接并且在没有任何先前声明的情况下被定义, const 内部链接, 链接说明符 C++, C中的链接有哪些类型, C 外部引用, C中 .... https://bing.com/search?q=%e7%bf%bb%e8%af%91%e5%8d%95%e5%85%83%e5%86%85%e9%83%a8+%e5%92%8c+%e5%a4%96%e9%83%a8.
(4) 具有外部链接并且在没有任何先前声明的情况下被定义, const 内部链接, 链接说明符 C++, C中的链接有哪些类型, C 外部引用, C中 .... https://zditect.com/article/10092990.html.
